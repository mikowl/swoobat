...
const PokeCard = ({ pokemon }: { pokemon: Pokemon }) => {
  ...
  return (
    <div className="flip" onClick={handleCardFlip}>
      <div className="front">
        <div
          className={`bg-white shadow-md text-secondary bg-opacity-60 px-2 py-2 rounded-xl transition-all hover:rotate-3 hover:-translate-y-1`}
        >
          <div className="h-[330px] bg-blue-200 p-3 relative rounded bg-opacity-70">
            ...
            <p className="mt-3 mb-2 text-sm leading-5">
              <b>Abilities: </b>
              {pokemon.abilities.map((ability) => ability.ability.name).join(", ")}
            </p>
            {/* <p className="mt-2 text-sm font-medium"> Base exp: {pokemon.base_experience} </p> */}
          </div>
        </div>
      </div>
      <div className="back">
        <div
          key={pokemon.id}
          className={`bg-white shadow-md text-secondary bg-opacity-60 px-2 py-2 rounded-xl transition-all`}
        >
