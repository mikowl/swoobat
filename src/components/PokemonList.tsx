...
// set useQuery to fetch the pokemon list
  ...
  return (
    <div>
      {isInitialLoading ? (
        <div role="status">
            <svg aria-hidden="true" className="w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-400 fill-yellow-200" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <>
            ...
            <div className="pagination mb-4 mt-4">
            ...
            <div className="grid justify-center grid-cols-4 gap-2 flex-wrap">
            {(data as Pokemon[]).map((pokemon) => (
              <div key={pokemon.id} className={`bg-[#565aa6] bg-opacity-95 px-4 py-3 rounded`}>
                <h3 className="capitalize mb-2">{pokemon.name} <small className="text-xs">{pokemon.id}</small></h3>
                ...
                <p>
                  <small>{pokemon.stats[0].stat.name}: {pokemon.stats[0].base_stat}</small>
                  {` / `}
                  <small>{pokemon.stats[1].stat.name}: {pokemon.stats[1].base_stat}</small>
                </p>
              </div>
            ))}
            </div>
